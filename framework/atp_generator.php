<?php
	class atpgenerator {
		...
		function sidebaroption($postid) {
			...
			if ( is_404() || is_tax() || is_singular( 'artists_type' )  || is_singular( 'genres' ) ||  is_singular( 'gallery_type' ) || is_singular( 'gallery' ) || is_singular( 'video_type' ) || is_singular( 'albums' ) ) {
				$sidebaroption = 'fullwidth';
			}
		...
		function subheader($postid){
			...
			if($subheaderbgcolor OR $subheaderimage) {
				$subheader_props = 'style="background:'.$subheaderbgcolor.$subheaderimage.'"';
				$subarrow_color = 'style="border-top-color:'.$subheaderbgcolor.'"';
			}else{
				$subheader_props = '';
			}
			$out = '';
